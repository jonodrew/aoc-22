...
def find_badge(rucksacks: Sequence[str]) -> str:
    return set(rucksacks[0]).intersection(*map(set, rucksacks[1:])).pop()


def part_one() -> int:
    return sum(map(lambda c: priorities().get(c), map(find_shared, read_in_from_file("day_03/input.txt"))))


def group_in_threes(rucksacks: Sequence[str]) -> Iterable[list[str]]:
...
def part_two() -> int:
    return sum(map(lambda c: priorities()[c], map(find_badge, group_in_threes(read_in_from_file("day_03/input.txt")))))
